...
extends RefCounted
const VERSION: int = 3
const PATH: String = "user://rescue-v3.json"
# Redesigned mechanisms are different puzzles. Leave v1/v2 files untouched.

static func validate(raw: Variant, count: int) -> Dictionary:
	var clean: Dictionary = {}
	if not raw is Dictionary or raw.get("version") != VERSION or not raw.get("best") is Dictionary:
		return clean
...
static func save_best(best: Dictionary) -> bool:
	...
	if file == null:
		return false
	file.store_string(JSON.stringify({"version":VERSION,"best":best}))
