...
func with_alpha(color:Color) -> Color:
	color.a*=painter_alpha;return color
...
func label(at:Vector2,text:String,color:Color=CREAM,font_size:int=12):draw_string(font,at,text,HORIZONTAL_ALIGNMENT_LEFT,-1,font_size,with_alpha(color))
...
func _draw():
	...
	for i in 140:
		...
		paint_rect(Rect2(pos,Vector2(2,2)),bg.lightened(0.06))
	scale_world=clampf(minf((size.x-70)/((dims.x+dims.y+12)*32.0),(size.y-95)/((dims.x+dims.y+12)*16.0+100)),0.45,1.4)*zoom
	origin=Vector2(size.x*0.5-(dims.x-dims.y-3)*16*scale_world,size.y*0.5-(dims.x+dims.y-3)*8*scale_world+34)+pan
	...
	box(-0.22,0,0.2,3,11,Color("42756b"),68)
	...
	for y in range(3,dims.y):
		...
		line(a+Vector2(0,-52),b+Vector2(0,-52),Color("42756b"),3)
	...
	for e in s.effects:
		...
		paint_rect(Rect2(pos-Vector2(width/2+5,14),Vector2(width+10,19)),Color("fff0c7"))
...
func items(f:Dictionary,x:float,y:float,z:float,row:int):
	...
	for i in 4:
		...
		var p=project(at+Vector2(0.07,0.16),z+3);paint_rect(Rect2(p,Vector2(2,2)),CREAM)
...
func draw_person(a:Dictionary,kind:String):
	...
	draw_texture_rect_region(atlas,dest,region,with_alpha(tint))
	...
	if not staff:
		...
		if accessory==0:
			paint_rect(Rect2(base+Vector2(-9,-42),Vector2(18,3)),Color("c18b59"))
			paint_rect(Rect2(base+Vector2(-6,-47),Vector2(12,5)),Color("dcb887"))
		elif accessory==1:paint_rect(Rect2(base+Vector2(9,-25),Vector2(6,12)),Color("ba7758"))
	...
	if show:
		...
		paint_rect(Rect2(p-Vector2(5,11),Vector2(width+10,19)),INK)
		paint_rect(Rect2(p-Vector2(3,9),Vector2(width+6,15)),CREAM)
