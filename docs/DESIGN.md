...
- 基準解法の成功や逆順操作の失敗だけを、難易度が適切である根拠にしない。初見の予測、失敗理由の理解、次の試行で変えた判断を確認する。
- 各面に「固有の判断」「もっともらしい誤解法と見える原因」「前の面の定石がそのまま通用しない理由」を記録する。左右反転、物体の増量、待ち順の追加だけを難易度の上昇として扱わない。
- 面名・見た目・左右を除いた因果関係で類似面を比較する。難易度や重複の再指摘があった場合は、物量の追加より先に面構成と採用基準を見直す。設計仮説と実際のプレイ結果を分けて報告する。
