...
人が空間を移動するゲームでは、作業中だけでなく、待機・作業終了・交代・退場でも位置と通路の意味を保つ。
待機場所は他の人の利用位置や待機列を塞がず、移動前後を画面上で追えるようにする。
同じ種類の詰まりが再発した場合は、正常な一往復だけでなく、通常プレイで問題が起きた状態から目的の完了までを再現して検証する。
