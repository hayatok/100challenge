...
- `art/source/yofukashi-kit.blend`: Blenderで確認・保存したモデル正本。
- `game/assets/models/`: 正本から出力した自己完結GLB 5種。manifestの正本SHA-256を検証します。
...
モデルの編集はBlenderの保存済み正本に対して行います。`scripts/create_art.py`は初回制作用で、既存正本を上書きしません。編集後の出力例:

```sh
/Applications/Blender.app/Contents/MacOS/Blender --background --python scripts/export_art.py
```

画像はこの環境のImageGenで制作しました。Google Imagen APIは使用していません。モデルはBlenderで生成・GUI調整し、色とシルエットを確認しました。アニメーションは上下動・方向転換までで、リグ付き歩行は今後の制作対象です。

## 次の開発

まず実プレイで囲みの成立感、被弾の分かりやすさ、180秒の難度を調整します。その後、ボスと製品版の時間設計、進化、3キャラ、章・会話、リグ付きアニメーション、対応OS拡大を進めます。企画書の12分ボス・14分敗北と今回の180秒リハーサルは別仕様です。
