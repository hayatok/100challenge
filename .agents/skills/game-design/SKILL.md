---
name: game-design
description: Design and critique a game's core loop, meaningful choices, difficulty, learning from failure, and feedback. Use for game planning, mechanic proposals, fun or dominant-strategy reviews, and playtest hypotheses. Consult the relevant design references before implementing gameplay decisions; engine implementation uses the project's Godot skills.
metadata:
  upstream_tags: "game, design"
  upstream_date: "2026-08-10"
  upstream_source_post: "game-mechanics"
---

# Game Design

## Project integration (Codex)

Read `docs/GAME_DEV_SKILLS.md` for this repository's application of this skill. User instructions, approved decisions, and project instructions take precedence over upstream procedures below.

- Use `$game-design` with natural language such as "critique the core loop"; upstream `/game-design` examples describe modes, not registered Codex commands.
- Native discovery uses `.agents/skills/`. The upstream `wire` mode and `INTEGRATION.md` describe optional Claude setup; do not execute that setup or create hooks/settings as part of using this skill unless the user explicitly requests it.
- Reuse the approved concept and existing project documents. Do not restart a concept interview, request repeated sign-off, or scaffold a large concept bundle for an already scoped change. Ask only about materially missing decisions.
- In this repository, the upstream `godot` role maps to `godot-gdscript-patterns` and `godot-physics`; stage construction uses `level-design`. Read only references relevant to the current task. Working alongside a skill does not require spawning agents.
- Treat frameworks as diagnostic tools. A finite physics puzzle does not require a meta progression, economy, inventory, or narrative system. Distinguish design hypotheses from observed playtest evidence; a document review alone does not establish that a game is fun.

The remainder is the upstream design procedure; local adaptation and provenance are recorded in `sources.json` one directory above this skill.
