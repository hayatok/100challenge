...
## 24. Version 1.2 — MLP / CNN分離とCNN観察室

Version 1.1の同時比較UIを廃止し、最初にMLPまたはCNNを選ぶ2階層の情報構造へ変更する。入力データの仕組みは共用するが、モード、trace、再生phase、説明、推論結果はモデルごとに独立させる。

### 24.1 Model navigation

- 初期状態ではモデルを選択せず、MLPまたはCNNの選択を最初の操作にする
- MLP: `1件を学ぶ`、`推論を見る`、`まとめて学ぶ`、`自分で書く`
- CNN: `特徴を観察`、`自分で書く`
- MLPとCNNのSoftmax値を同じ表で比較しない
- モデル切替時は進行中の再生と表示traceを解除する

### 24.2 CNN spatial probe

- 特徴マップの任意座標をクリックまたは矢印キーで選択できる
- 選択座標から入力画像上の受容野を算出して連動表示する
- Conv1では入力3×3、カーネル3×3、各積、bias、合計、ReLU後を表示する
- Conv2では選択入力チャネルの3×3小計と、全8チャネルを含む合計を分けて表示する
- Pool1では対応する2×2の4値と、採用された最大値を表示する

### 24.3 Evidence and counterfactual

- 選択数字について、Pool2の各7×7位置からDenseへの全16チャネル寄与を合計し、正負の証拠マップとして表示する
- 49位置の寄与とbiasの合計が、保存traceのlogitと丸め誤差内で一致する
- 選択中の入力受容野を0で遮蔽し、同じCNNで再推論して予測と確率の変化を表示する
- 遮蔽結果を元traceへ混在させない

### 24.4 Display scale

- `各MAP`: MAPごとの最大絶対値を明るさ100%とし、形を観察しやすくする
- `共通尺度`: 同じ層の全MAPで共通最大値を使い、反応強度を比較可能にする
- 現在の尺度と意味を文言でも表示し、色だけに依存しない

### 24.5 Acceptance criteria

- 局所Conv、Pool、証拠mapが実traceと数値整合する
- 遮蔽関数が指定範囲だけを変更する
- MLPの既存学習・推論・手書き機能を維持する
- 375px、768px、1024px、1440pxでモデル選択とCNN観察操作が成立する

## 25. Version 1.3 — Live network map

MLPとCNNの観察画面へ、入力から回答までの全体構造を示す模式図を追加する。既存のノード図と特徴マップは詳細観察として維持し、模式図は現在位置を示す全体地図として扱う。

### 25.1 Structure

- MLP: `入力画像 → Flatten → 全結合16 → ReLU → Logits 10 → Softmax → 回答`
- CNN: `入力画像 → Conv1 → Pool1 → Conv2 → Pool2 → Flatten → Dense 10 → Softmax / 回答`
- 784入力やすべての接続線は常時展開せず、shapeと演算名で束として示す

### 25.2 Trace synchronization

- 現在、通過済み、未到達の層を色だけでなく文言と輪郭で区別する
- 各層に実trace由来の活性数、最大値、確率、予測クラスを表示する
- 再生phaseの変更と模式図の現在位置を同期する
- 模式図の層を選択すると対応phaseへ移動し、既存の詳細可視化も同時に切り替わる
- MLPの逆伝播phaseでは進行方向を `BACKPROP ←` として表示する

### 25.3 Motion and accessibility

- 現在の接続に短いデータパケットを流し、演算の方向を示す
- `prefers-reduced-motion` ではパケット移動を停止しても、現在位置の文言と輪郭を残す
- 開始前にも回路全体を表示するが、層移動操作はtrace生成まで無効にする

## 26. Version 1.4 — CNN training and inference modes

CNNにもMLPと同じく、更新可能な学習モデルと固定された学習済みモデルを分けて用意する。

- `1件を学ぶ`: seed固定の未学習CNNへbatch size 1でcross entropyとSGD 0.01を適用する
- `推論を見る`: 学習済みCNNを変更せずに推論・特徴観察する
- 学習traceは更新前後の全CNN trace、loss、全9,098パラメータの平均絶対勾配と平均絶対更新量、revisionを保持する
- 再生は順伝播、正解比較、loss、逆伝播、更新、更新後再推論の順とする
- 学習前へ戻す操作は直前の全CNNパラメータを復元する
- 学習用CNNと学習済みCNNの状態を混在させない

## 27. Version 1.5 — CNN principle map and benchmark

- 工程カードに加え、入力面、特徴マップ群、Pooling、ベクトル、出力ノードを複数接続線で結ぶ全体構造図を表示する
- 黄色い枠は受容野であり、選択した特徴マップ1点へ影響する元画像範囲だと図中と詳細パネルの両方で説明する
- CNNの1件学習前後に、学習と分離したtest subset 1,000件を全件推論してaccuracyを記録する
- CNNのまとめ学習は10、50、100件を選択でき、10件ごとの実batch lossとtest accuracyを記録する
- グラフではlossとaccuracyを別色・凡例付きで同時表示し、先頭精度と最新精度を数値でも示す

## 28. Version 1.6 — 3D-first CNN observation

- CNNの主要表示をThree.jsによる3Dネットワークへ置き換える
- 入力、Conv1、Pool1、Conv2、Pool2は実traceをテクスチャ化した面の束として描画する
- Flattenは784値の束、Denseと回答は10ノードとして同一空間に配置し、全層を複数線で接続する
- 黄色い入力受容野と選択特徴セルを線で結び、現在phaseの層を黄色い輪郭と移動パケットで示す
- `全体を見る` と `現在へ寄る` のカメラを用意し、層ラベルと3Dオブジェクトの選択を再生phaseへ同期する
- CNNで重複していた2D工程図と空の説明領域は表示しない
- 特徴マップ、局所積和、Pooling、根拠、遮蔽、学習ベンチマークは3Dを検証する補助パネルとして維持する
