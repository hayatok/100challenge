...
const modes={import:['--editor','--import','--quit'],test:['--fixed-fps','60','--script','res://tests/run.gd'],ui:['--fixed-fps','60','--script','res://tests/ui.gd'],build:['--export-release','Web'],debug:['--export-debug','Web']};
if(command==='editor'||command==='play'){
 const child=spawn(executable,['--path',project,...(command==='editor'?['--editor','--accessibility','always']:[])],{stdio:'ignore',detached:true});child.unref();
}else{
 ...
 if(['build','debug'].includes(command))copyFileSync(path.join(root,'racing-cover.png'),path.join(root,'dist/racing-cover.png'));
}
